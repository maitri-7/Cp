...
void solve(){
    ...
    rep(i,0,n) cin>>b[i];
    vector<ll> pref(n+1);
    rep(i,0,n) pref[i+1] = pref[i] + (a[i]*b[i]);
    //considering all the subsets
    ll ans = pref[n];
    rep(i,0,n){
        // considering all the subsegments of odd lengths
        ll curr = a[i]*b[i];//considering curr element as the center
        //Thus it will be multiplied with itself and all other elements will be cross multiplied
